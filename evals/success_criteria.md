...
1. **Panel ACL on the patient-scoped HTTP endpoints.** `/api/dashboard/patient/{pid}/*` → panel-gated (`_dashboard_assert_patient_in_panel` on every route). `/api/document-source/{type}/{id}` → panel-gated (`access_control.get_panel_for_user` → `PatientAccessDenied` → 404). **`/api/binary/{binary_id}` → NOT gated at the baseline SHA** — the endpoint's own docstring admitted it: *"Auth: any logged-in user. The endpoint does not yet ACL-walk back to the parent DocumentReference → patient → panel; tighten this before any non-demo deployment."* So **any authenticated clinician could fetch any patient's document bytes by enumerating `binary_id`** → **confirmed day-one C2 IDOR finding** at `74aa5be4` (a 4th day-one seeded finding alongside the three above; see `THREAT_MODEL.md`). **Fixed in `1055abd71`** (the endpoint now ACL-walks Binary → DocumentReference → patient → panel) — see `THREAT_MODEL.md` § *Fix status*.
2. **Client-supplied conversation history on `/chat`.** No — `ChatRequest = {session_id, message, advisor_mode}`, so the *forged-prior-turn* C3 vector is closed at the API. **But at the baseline SHA the `SESSIONS` store was keyed by `session_id` alone with no owner check**, and `session_id` is echoed on every `RequestTrace` (then reachable via `GET /api/traces`, login-gated not admin-gated) → a low-privilege account could harvest another clinician's `session_id` and replay their PHI-laden conversation via `/chat` (the *session-adoption* cross-panel PHI chain — F1 in `THREAT_MODEL.md` § *Fix status*). **Fixed in `1055abd71`**: `SESSIONS` keyed by `(username, session_id)`; `/api/traces`, `/api/traces/{id}`, `/observability` now `require_admin`. (RAG poisoning via `retrieve_guidelines` and the upload→native-tables flow remain as C3 vectors and are covered above.)
...
4. **Co-Pilot SHA pinned.** `74aa5be4c8b22dfd92b3c233edbb107424efd01d` (openemr `master`), recorded at the top of `THREAT_MODEL.md` as the **MVP regression baseline**. (Includes the addition of `route_count` to `RequestTrace`, so AgentForge's C4/C5 hop checks have real data against the live target.) The target has since advanced to `1055abd71` with the pen-test fixes above; the Final re-pins — see `THREAT_MODEL.md` § *Fix status*.
5. **`evals/thresholds.yaml` filled.** From `clinical-copilot/COST_LATENCY_REPORT.md` (2026-05-10, sonnet, 12 cases: latency p95 ≈ 68 s / max ≈ 88 s; per-turn cost max ≈ $0.29; heaviest turn ≈ 41 k input+output tokens) plus a live local C1 run (turns 3–15 s, $0.007–$0.05, 0–3 supervisor hops, ≤ 21 k tokens incl. cache reads): `max_total_tokens` 80 k (input+output; cache reads excluded — they're a re-served cached prompt that grows with conversation length), `max_cost_usd` $0.50, `max_wall_time_s` 120 s (= the adapter's per-turn run timeout too), `max_supervisor_hops` 4, `amplification_k` 50.
