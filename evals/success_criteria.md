...
1. **Panel ACL on the patient-scoped HTTP endpoints.** `/api/dashboard/patient/{pid}/*` → panel-gated (`_dashboard_assert_patient_in_panel` on every route). `/api/document-source/{type}/{id}` → panel-gated (`access_control.get_panel_for_user` → `PatientAccessDenied` → 404). **`/api/binary/{binary_id}` → NOT gated** — the endpoint's own docstring: *"Auth: any logged-in user. The endpoint does not yet ACL-walk back to the parent DocumentReference → patient → panel; tighten this before any non-demo deployment."* So **any authenticated clinician can fetch any patient's document bytes by enumerating `binary_id`** → **confirmed day-one C2 IDOR finding** (a 4th day-one seeded finding alongside the three above; see `THREAT_MODEL.md`).
2. **Client-supplied conversation history on `/chat`.** No — `ChatRequest = {session_id, message, advisor_mode}`; prior turns live server-side in the `SESSIONS` dict keyed by `session_id`. The forged-prior-turn C3 vector is closed at the API. (RAG poisoning via `retrieve_guidelines` and the upload→native-tables flow remain as C3 vectors and are covered above.)
...
4. **Co-Pilot SHA pinned.** `74aa5be4c8b22dfd92b3c233edbb107424efd01d` (openemr `master`), recorded at the top of `THREAT_MODEL.md`. (Includes the addition of `route_count` to `RequestTrace`, so AgentForge's C4/C5 hop checks have real data against the live target.)
