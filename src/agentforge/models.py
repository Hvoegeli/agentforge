...
class TargetSurface(StrEnum):
    ...
    DASHBOARD_API = "dashboard_api"        # /api/dashboard/patient/{pid}/*, /api/binary/{id}, /api/document-source/*
    ADMIN_API = "admin_api"               # /api/admin/*
    RETRIEVAL_CORPUS = "retrieval_corpus"  # the USPSTF/ADA guideline corpus behind retrieve_guidelines
...
class AttackSource(StrEnum):
    ...
    SEEDED_FINDING = "seeded_finding"   # a known vulnerability we pre-loaded (zero-citation bypass, etc.)
...
class AttackCase(BaseModel):
    ...
    framework_refs: list[str] = Field(default_factory=list)  # e.g. ["OWASP-LLM01", "ATLAS-AML.T0051", "NIST-AI600-1:InfoSec"]
    source: AttackSource
    in_regression_suite: bool = False             # promoted only after a confirmed/approved finding
    severity_hint: Severity | None = None         # the Red Team's a-priori guess; the Finding carries the real one
    notes: str = ""
...
class JudgeVerdict(BaseModel):
    ...
    evidence_links: list[str] = Field(default_factory=list)  # pointers into the trace / response that justify the verdict
    judge_model: str | None = None                # which model rendered the verdict (None for pure-deterministic)
    judge_prompt_version: str | None = None       # so a verdict can be re-validated when the Judge prompt changes
...
class Finding(BaseModel):
    ...
    framework_mapping: list[str] = Field(default_factory=list)  # OWASP / ATLAS / NIST refs for the vuln report
...
class RunRecord(BaseModel):
    ...
    id: str = Field(default_factory=_new_id)
    orchestrator_directive: str                   # e.g. "probe under-tested category C2 against local target, budget $0.40"
    ...
    halted_reason: str | None = None              # "budget_reached" / "no_signal" / "target_unavailable" / "human_stop" / None
