...
| How does AgentForge reach the Co-Pilot? | Only through [`src/agentforge/target/adapter.py`](src/agentforge/target/adapter.py) — an `httpx.Client` against an allowlisted external URL. There is **no shared module import**, **no shared database**, **no shared Python process**. The allowlist enforcement is at [`adapter.py:179–186`](src/agentforge/target/adapter.py#L179-L186) (`is_allowed_target_host()` raises `TargetNotAllowedError` for anything off-allowlist). |
...
| [`evals/results/c4-c6-live-1055abd71.json`](evals/results/c4-c6-live-1055abd71.json) | A single live campaign run record (RunRecord) against the deployed Co-Pilot, including UTC timestamps. | `"target_base_url"` · `"target_sha"` · `"started_at"` · `"finished_at"` · `"n_attacks"` |
