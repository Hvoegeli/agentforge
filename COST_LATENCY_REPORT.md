...
|---|---|---|---|
| **~100 runs** (measured baseline) | Full model set, no caching | **~$0.0014** | The 20-run live sample averages **$0.0014/run** of *AgentForge LLM spend* (Red Team mutation + LLM-Judge + Documentation narrative; this is the budget number, what AgentForge actually pays via OpenRouter). The Co-Pilot's own inference bill — recorded for visibility, AgentForge does not pay — runs ~$0.031/run; that scales with the target's own pricing, not AgentForge's architecture. Most of AgentForge's spend in this sample is the LLM-Judge escalating UNCERTAIN verdicts (Mistral-Large-2411 at $2/$6 per Mtok). |
